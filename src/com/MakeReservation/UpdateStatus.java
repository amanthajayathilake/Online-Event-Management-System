...
public class UpdateStatus extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		isTrue=RetrieveBookingDBUtil.updatebooking(id, name, pkg, phone, mail,bdate ,snote,ss);
		
		if (isTrue==true) { //if update is success then boolean value is true and navigate to the following page
			
			List<BookingDetails> bkDetails= RetrieveBookingDBUtil.getBooking(name);
			request.setAttribute("bkDetails", bkDetails);            //pass updated values to the BookingDetails.jsp
			
			RequestDispatcher dis = request.getRequestDispatcher("HomePGmessageSuccess.jsp");
			...
			RequestDispatcher dis = request.getRequestDispatcher("HomePGmessageUNSuccess.jsp");
			dis.forward(request, response);
		}
	}
